...
( cd services/mcp-server
  gcloud run deploy "$MCP_SVC" --source=. --region="$GCP_REGION" \
    ...
    --set-env-vars="^@^GH_OWNER=${GH_OWNER}@CONTENT_REPO=${CONTENT_REPO}@GH_BRANCH=${GH_BRANCH}@POSTS_DIR=${POSTS_DIR}@AUTHORS_DIR=${AUTHORS_DIR}@AUTHORS_DATA=${AUTHORS_DATA}@IMAGES_DIR=${IMAGES_DIR}@TAGS_DATA=${TAGS_DATA:-_data/tags.yml}@TOOL_PREFIX=post_" \
